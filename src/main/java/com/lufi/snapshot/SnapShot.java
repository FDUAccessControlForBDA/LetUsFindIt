...
public class SnapShot {
    ...
    public String getReport(ArrayList<MatchInfo> matchList, Converter con) throws IOException {
        ...
        // 初始化输出文件
        String reportName = FilenameUtils.getFullPath(fileName) + FilenameUtils.getBaseName(fileName) + "_report.txt";
        ...
        try{
            buffer = splitLine
                    ...
                    + "#隐私信息所占比（行数/总行数）：" + (matchList.size() / con.getLines() * 100) + "%" + System.getProperty("line.separator")
                    + splitLine
